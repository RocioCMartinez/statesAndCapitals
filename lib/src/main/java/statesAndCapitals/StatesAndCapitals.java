...
public class StatesAndCapitals
{
    ...
    public void testStatesAndCapitals() throws IOException
    {
        ...
        List<StateInfo> lastFiveStates = states.stream().skip(Math.max(0, states.size() - 5)).toList();
        ...
        List<Integer> lastFiveNumbers = Stream.of( 16, 17, 18, 19, 20).collect(toList());
        ...
        StateInfo cardinalState = states.stream().filter(state -> state.getStateBird().equalsIgnoreCase("cardinal")).findFirst().orElseThrow(() -> new NoSuchElementException("No state with cardinal as the state bird"));
        ...
        //ChatGPT helped me split and find regex.

        Boolean doNoStatesHaveAOneWordMotto = states.stream().noneMatch(state -> state.getStateMotto() != null && state.getStateMotto().split(" ").length ==1);
        ...
        //Aaron Clark helped with A11 and A12 as I was struggling to tap into the CapitalInfo.

        Double averageYearlyPrecipitationAcrossStateCapitals =states.stream().map(StateInfo::getCapital).collect(averagingDouble(CapitalInfo::getAverageYearlyPrecipitationInInches));
        //Could not implement without changing CapitalInfo to static
        //states.stream().mapToDouble(CapitalInfo::getAverageYearlyPrecipitationInInches).average().orElse(0.0)
        ...
        List<String> allDistinctStateBirdsMinusMockingbirds = states.stream().map(StateInfo::getStateBird).filter(bird -> !bird.contains("mockingbird")).distinct().collect(Collectors.toList());
